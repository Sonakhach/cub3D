...
void	find_pos_player(t_addres *address)
{
	...
	i = 0;
	while (address->cub->map[i])
	{
		j = 0;
		while (address->cub->map[i][j])
		{
			if (address->cub->map[i][j] == 'N' || \
				address->cub->map[i][j] == 'S' || \
				address->cub->map[i][j] == 'E' || \
				address->cub->map[i][j] == 'W')
			{
				init_player_pos(address, i, j);
				address->rcasting->posx = i + 0.5;
				address->rcasting->posy = j + 0.5;
				address->cub->map[i][j] = '0';
				return ;
			}
			j++;
		}
...
void	norm_init_win_2(t_addres *address)
{
	...
	while (address->rcasting->hit == 0)
	{
		...
		if (address->cub->map[address->rcasting->mapx] \
